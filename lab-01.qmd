...
```{r}
flow_meta$raw_rank <- rank(-flow_meta$Flow)
flow_meta$rank <- rank(-flow_meta$flow_per_sqmi)
...
# e.g., annual |> left_join(site_meta |> select(site_no, dec_lat_va, dec_long_va), by = "site_no")

# Verify the join
# nrow(result) == nrow(annual)?
# sum(is.na(result$dec_lat_va)) == 0?
```
...
lees_model_data <- annual |>
  ...
  left_join(annual |> filter(site_no == "09085000") |> rename(glenwood_flow = total_flow),
            by = "water_year") |>
